...
async function seedEntities(client: Client): Promise<void> {
  for (const e of ENTITIES) {
    ...
    const data = JSON.stringify({ name: e.name, description: e.description });
    // entity_versions has a BEFORE INSERT SCD2 trigger (set_entity_version_validity)
    // that rewrites sibling rows, which clashes with INSERT ... ON CONFLICT
    // ("cannot affect row a second time"). Update-first / insert-if-missing keeps
    // this idempotent without firing the insert trigger on an existing version.
    const updated = await client.query(
      `UPDATE entity_versions
         SET data = $2::jsonb, is_current = true, updated_at = now()
       WHERE entity_id = $1 AND version_number = 1`,
      [entityId, data],
    );
